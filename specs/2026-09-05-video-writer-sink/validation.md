...
- [ ] The **same** command in a **default** build fails fast, before processing
      any frame, naming `--output_video` and
      `-DNEURIPLO_INFER_WITH_VIDEOWRITER=ON`.
- [ ] `--output_video=/tmp/out.mp4 --source=<image.jpg>` is rejected in **both**
      builds with an error naming `--output_video` and saying it applies to
      video sources only.

## Automated

- [ ] `toFrame` round-trip (default build): a BGR8 `cv::Mat` converted to a
      ...
      to an empty `Frame`.
- [ ] Writer round-trip (writer build, `#ifdef VIDEOCAPTURE_WITH_WRITER`):
...
- [ ] CLI parse tests (both builds, unless gated):
...
- [ ] Capability advertisement (writer build): the `parameters` map in
...
- [ ] Capability **non**-advertisement (default build): `--capabilities` output
      is byte-identical to `develop` (the catalog entry and task references
      are compiled out).
- [ ] `capabilities_cli_contract` and `capabilities_schema_contract` pass in
...
- [ ] Nothing in *Out of Scope* was implemented anyway — in particular, no
      ...
      `schema_version` bump.
- [ ] The default build's `--capabilities` output is byte-identical to before.
- [ ] `videocapture` is pinned to `v0.5.0` in `versions.env`.
- [ ] Deviations from this file are recorded here, honestly, with what was run
      instead.
- [ ] `CHANGELOG.md` has an `Added` entry for `--output_video` and a `Changed`
